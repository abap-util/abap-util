...
CLASS ltcl_number_conv IMPLEMENTATION.
  ...
  METHOD num2str_decimal_2.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `3.14`
                                        act = zabaputil_cl_util_context=>conv_number_to_string( val = temp45 decimals = 2 ) ).
  ENDMETHOD.
  ...
  METHOD num2str_decimal_0.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `3`
                                        act = zabaputil_cl_util_context=>conv_number_to_string( val = temp46 decimals = 0 ) ).
  ENDMETHOD.
...
CLASS ltcl_date_conv IMPLEMENTATION.
  ...
  METHOD date_to_str_default.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `2024-03-15`
                                        act = zabaputil_cl_util_context=>conv_date_to_string( temp57 ) ).
  ENDMETHOD.
  ...
  METHOD date_to_str_dd_mm_yyyy.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `15.03.2024`
                                        act = zabaputil_cl_util_context=>conv_date_to_string( val = temp58
                                                                                   format = `DD.MM.YYYY` ) ).
  ENDMETHOD.
  ...
  METHOD date_to_str_custom_sep.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `03/15/2024`
                                        act = zabaputil_cl_util_context=>conv_date_to_string( val = temp59
                                                                                   format = `MM/DD/YYYY` ) ).
  ENDMETHOD.
...
CLASS ltcl_filter_ops IMPLEMENTATION.
  ...
  METHOD sql_where_eq.
    DATA temp61 TYPE zabaputil_cl_util_context=>ty_t_filter_multi.
  ...
  METHOD sql_where_bt.
    DATA temp63 TYPE zabaputil_cl_util_context=>ty_t_filter_multi.
...
DATA lt_tab TYPE temp2.
    ...
    DATA temp97 TYPE zabaputil_cl_util_context=>ty_t_filter_multi.
...
CLASS ltcl_time_ops IMPLEMENTATION.
  ...
  METHOD stampl_by_date_time.
    ...
    lv_ts = zabaputil_cl_util_context=>time_get_stampl_by_date_time( date = temp177
                                                                time = temp29 ).
    cl_abap_unit_assert=>assert_not_initial( lv_ts ).
  ENDMETHOD.
...
CLASS ltcl_calendar_ops IMPLEMENTATION.
  ...
  METHOD weekday_monday.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1
        act = zabaputil_cl_util_context=>cal_get_weekday( temp188 ) ).
  ENDMETHOD.
  ...
  METHOD weekday_sunday.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 7
        act = zabaputil_cl_util_context=>cal_get_weekday( temp189 ) ).
  ENDMETHOD.
  ...
  METHOD weekday_wednesday.
    ...
    temp190 = `20240103`.
    cl_abap_unit_assert=>assert_equals( exp = 3
        act = zabaputil_cl_util_context=>cal_get_weekday( temp190 ) ).
  ENDMETHOD.
  ...
  METHOD weekday_saturday.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 6
        act = zabaputil_cl_util_context=>cal_get_weekday( temp191 ) ).
  ENDMETHOD.
  ...
  METHOD is_weekend_saturday.
    ...
    temp192 = `20240106`.
    cl_abap_unit_assert=>assert_true(
        zabaputil_cl_util_context=>cal_is_weekend( temp192 ) ).
  ENDMETHOD.
  ...
  METHOD is_weekend_sunday.
    ...
    temp193 = `20240107`.
    cl_abap_unit_assert=>assert_true(
        zabaputil_cl_util_context=>cal_is_weekend( temp193 ) ).
  ENDMETHOD.
  ...
  METHOD is_weekend_friday.
    ...
    temp194 = `20240105`.
    cl_abap_unit_assert=>assert_false(
        zabaputil_cl_util_context=>cal_is_weekend( temp194 ) ).
  ENDMETHOD.
  ...
  METHOD count_workdays_full_week.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = 5
        act = zabaputil_cl_util_context=>cal_count_workdays( date_from = temp198
                                                      date_to   = temp33 ) ).
  ENDMETHOD.
  ...
  METHOD count_workdays_same_day.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = 0
        act = zabaputil_cl_util_context=>cal_count_workdays( date_from = temp199
                                                      date_to   = temp34 ) ).
  ENDMETHOD.
...
CLASS ltcl_zip_ops IMPLEMENTATION.
  ...
  METHOD pack_basic.
    ...
    DATA temp200 TYPE zabaputil_cl_util_context=>ty_t_zip_file.
    DATA temp201 LIKE LINE OF temp200.
    DATA temp35 TYPE xstring.
    DATA lt_files LIKE temp200.
    ...
    temp201-name = `test.txt`.
    
    temp35 = `48656C6C6F`.
    temp201-content = temp35.
  ...
  METHOD pack_empty.
    DATA lo_probe TYPE REF TO object.
    DATA temp202 TYPE zabaputil_cl_util_context=>ty_t_zip_file.
    DATA lt_files LIKE temp202.
    DATA lv_archive TYPE xstring.
    TRY.
        CREATE OBJECT lo_probe TYPE ('CL_ABAP_ZIP').
      CATCH cx_root.
  ...
  METHOD roundtrip.
    DATA lo_probe TYPE REF TO object.
    DATA temp203 TYPE zabaputil_cl_util_context=>ty_t_zip_file.
    DATA temp204 LIKE LINE OF temp203.
    DATA temp36 TYPE xstring.
    DATA temp37 TYPE xstring.
    DATA lt_in LIKE temp203.
    DATA lv_archive TYPE xstring.
    DATA lt_out TYPE zabaputil_cl_util_context=>ty_t_zip_file.
    DATA ls_in LIKE LINE OF lt_in.
      DATA temp205 TYPE zabaputil_cl_util_context=>ty_s_zip_file.
      DATA temp206 TYPE zabaputil_cl_util_context=>ty_s_zip_file.
      DATA ls_out LIKE temp205.
    TRY.
        CREATE OBJECT lo_probe TYPE ('CL_ABAP_ZIP').
      CATCH cx_root.
        RETURN.
    ...
    temp36 = `414141`.
    temp204-content = temp36.
    INSERT temp204 INTO TABLE temp203.
    temp204-name = `b.txt`.
    
    temp37 = `424242`.
    temp204-content = temp37.
    INSERT temp204 INTO TABLE temp203.
    ...
    LOOP AT lt_in INTO ls_in.
      ...
      READ TABLE lt_out INTO temp206 WITH KEY name = ls_in-name.
      IF sy-subrc = 0.
        temp205 = temp206.
      ENDIF.
...
CLASS ltcl_xlsx_ops IMPLEMENTATION.
  ...
  METHOD xlsx_by_itab_basic.
    ...
    TYPES temp20 TYPE STANDARD TABLE OF ty_row WITH DEFAULT KEY.
DATA lt_tab TYPE temp20.
    DATA temp207 LIKE lt_tab.
...
CLASS ltcl_itab_aggregations IMPLEMENTATION.
  ...
  METHOD get_flights.
    DATA temp209 TYPE ltcl_itab_aggregations=>ty_t_flight.
  ...
  METHOD distinct.
    ...
    DATA temp213 LIKE LINE OF lt_distinct.
    DATA temp214 LIKE sy-tabix.
    DATA temp215 LIKE LINE OF lt_distinct.
    ...
    temp214 = sy-tabix.
    READ TABLE lt_distinct INDEX 1 INTO temp213.
    sy-tabix = temp214.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp216 = sy-tabix.
    READ TABLE lt_distinct INDEX 2 INTO temp215.
    sy-tabix = temp216.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD group_sum_by.
    ...
    DATA temp217 LIKE LINE OF lt_sums.
    DATA temp218 LIKE sy-tabix.
    DATA temp219 LIKE LINE OF lt_sums.
    DATA temp220 LIKE sy-tabix.
    DATA temp221 LIKE LINE OF lt_sums.
    DATA temp222 LIKE sy-tabix.
    DATA temp223 LIKE LINE OF lt_sums.
    ...
    temp218 = sy-tabix.
    READ TABLE lt_sums INDEX 1 INTO temp217.
    sy-tabix = temp218.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp220 = sy-tabix.
    READ TABLE lt_sums INDEX 1 INTO temp219.
    sy-tabix = temp220.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp222 = sy-tabix.
    READ TABLE lt_sums INDEX 2 INTO temp221.
    sy-tabix = temp222.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp224 = sy-tabix.
    READ TABLE lt_sums INDEX 2 INTO temp223.
    sy-tabix = temp224.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
...
CLASS ltcl_num_round IMPLEMENTATION.
  ...
  METHOD round_half_up.
    ...
    DATA temp226 TYPE decfloat34.
    DATA temp39 TYPE decfloat34.
    temp225 = '2.5'.
    
    temp38 = 3.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( temp225 )
        exp = temp38 ).
    ...
    temp39 = 2.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( temp226 )
        exp = temp39 ).
  ENDMETHOD.

  METHOD round_half_up_negative.
    DATA temp227 TYPE decfloat34.
    DATA temp40 TYPE decfloat34.
    temp227 = '-2.5'.
    
    temp40 = -3.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( temp227 )
        exp = temp40 ).
  ENDMETHOD.
  ...
  METHOD round_up.
    ...
    temp41 = 3.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( val  = temp228
                                                    mode = `UP` )
        exp = temp41 ).
  ENDMETHOD.
  ...
  METHOD round_down.
    ...
    temp42 = 2.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( val  = temp229
                                                    mode = `DOWN` )
        exp = temp42 ).
  ENDMETHOD.
  ...
  METHOD round_two_decimals.
    ...
    temp43 = '2.35'.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>num_round( val      = temp230
                                                    decimals = 2 )
        exp = temp43 ).
  ENDMETHOD.
...
CLASS ltcl_cur_amounts IMPLEMENTATION.
  ...
  METHOD to_external_zero_decimals.
    ...
    DATA temp231 TYPE decfloat34.
    DATA temp44 TYPE decfloat34.
    temp231 = '123.45'.
    
    temp44 = 12345.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>cur_amount_to_external( val      = temp231
                                                                 decimals = 0 )
        exp = temp44 ).
  ENDMETHOD.
  ...
  METHOD to_external_three_decimals.
    ...
    DATA temp232 TYPE decfloat34.
    DATA temp45 TYPE decfloat34.
    temp232 = '123.45'.
    
    temp45 = '12.345'.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>cur_amount_to_external( val      = temp232
                                                                 decimals = 3 )
        exp = temp45 ).
  ENDMETHOD.

  METHOD to_external_two_decimals.
    DATA temp233 TYPE decfloat34.
    DATA temp46 TYPE decfloat34.
    temp233 = '123.45'.
    
    temp46 = '123.45'.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>cur_amount_to_external( val      = temp233
                                                                 decimals = 2 )
        exp = temp46 ).
  ENDMETHOD.

  METHOD to_internal_zero_decimals.
    DATA temp234 TYPE decfloat34.
    DATA temp47 TYPE decfloat34.
    temp234 = 12345.
    
    temp47 = '123.45'.
    cl_abap_unit_assert=>assert_equals(
        act = zabaputil_cl_util_context=>cur_amount_to_internal( val      = temp234
                                                                 decimals = 0 )
        exp = temp47 ).
  ENDMETHOD.

  METHOD roundtrip.
    DATA temp235 TYPE decfloat34.
    DATA lv_val LIKE temp235.
    temp235 = '987.65'.
