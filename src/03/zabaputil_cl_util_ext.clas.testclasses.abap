...
CLASS ltcl_lock_ops IMPLEMENTATION.
  ...
  METHOD dequeue_from_enqueue.
    cl_abap_unit_assert=>assert_equals(
        exp = `DEQUEUE_EVVBAKE`
        act = zabaputil_cl_util_ext=>lock_get_dequeue_by_enqueue( `ENQUEUE_EVVBAKE` ) ).
  ENDMETHOD.
  ...
  METHOD dequeue_already_dequeue.
    cl_abap_unit_assert=>assert_equals(
        exp = `DEQUEUE_EVVBAKE`
        act = zabaputil_cl_util_ext=>lock_get_dequeue_by_enqueue( `DEQUEUE_EVVBAKE` ) ).
  ENDMETHOD.

  METHOD dequeue_short_name.
    cl_abap_unit_assert=>assert_equals(
        exp = `DEQUEUE_EMARA`
        act = zabaputil_cl_util_ext=>lock_get_dequeue_by_enqueue( `ENQUEUE_EMARA` ) ).
  ENDMETHOD.
...
CLASS ltcl_calendar_ops IMPLEMENTATION.
  ...
  METHOD weekday_monday.
    cl_abap_unit_assert=>assert_equals( exp = 1
        act = zabaputil_cl_util_ext=>cal_get_weekday( CONV d( `20240101` ) ) ).
  ENDMETHOD.

  METHOD weekday_sunday.
    cl_abap_unit_assert=>assert_equals( exp = 7
        act = zabaputil_cl_util_ext=>cal_get_weekday( CONV d( `20240107` ) ) ).
  ENDMETHOD.

  METHOD weekday_wednesday.
    cl_abap_unit_assert=>assert_equals( exp = 3
        act = zabaputil_cl_util_ext=>cal_get_weekday( CONV d( `20240103` ) ) ).
  ENDMETHOD.

  METHOD weekday_saturday.
    cl_abap_unit_assert=>assert_equals( exp = 6
        act = zabaputil_cl_util_ext=>cal_get_weekday( CONV d( `20240106` ) ) ).
  ENDMETHOD.

  METHOD is_weekend_saturday.
    cl_abap_unit_assert=>assert_true(
        zabaputil_cl_util_ext=>cal_is_weekend( CONV d( `20240106` ) ) ).
  ENDMETHOD.

  METHOD is_weekend_sunday.
    cl_abap_unit_assert=>assert_true(
        zabaputil_cl_util_ext=>cal_is_weekend( CONV d( `20240107` ) ) ).
  ENDMETHOD.

  METHOD is_weekend_friday.
    cl_abap_unit_assert=>assert_false(
        zabaputil_cl_util_ext=>cal_is_weekend( CONV d( `20240105` ) ) ).
  ENDMETHOD.

  METHOD add_workdays_skip_weekend.
    cl_abap_unit_assert=>assert_equals(
        exp = CONV d( `20240318` )
        act = zabaputil_cl_util_ext=>cal_add_workdays( date = CONV d( `20240315` ) days = 1 ) ).
  ENDMETHOD.

  METHOD add_workdays_mid_week.
    cl_abap_unit_assert=>assert_equals(
        exp = CONV d( `20240104` )
        act = zabaputil_cl_util_ext=>cal_add_workdays( date = CONV d( `20240102` ) days = 2 ) ).
  ENDMETHOD.

  METHOD add_workdays_zero.
    cl_abap_unit_assert=>assert_equals(
        exp = CONV d( `20240315` )
        act = zabaputil_cl_util_ext=>cal_add_workdays( date = CONV d( `20240315` ) days = 0 ) ).
  ENDMETHOD.

  METHOD count_workdays_full_week.
    cl_abap_unit_assert=>assert_equals(
        exp = 5
        act = zabaputil_cl_util_ext=>cal_count_workdays( date_from = CONV d( `20240101` )
...
CLASS ltcl_zip_ops IMPLEMENTATION.
  ...
  METHOD pack_basic.
    ...
    DATA(lt_files) = VALUE zabaputil_cl_util_ext=>ty_t_zip_file(
        ( name = `test.txt` content = CONV xstring( `48656C6C6F` ) ) ).
    DATA(lv_archive) = zabaputil_cl_util_ext=>zip_pack( lt_files ).
  ...
  METHOD roundtrip.
    ...
    DATA(lt_in) = VALUE zabaputil_cl_util_ext=>ty_t_zip_file(
        ( name = `a.txt` content = CONV xstring( `414141` ) )
        ( name = `b.txt` content = CONV xstring( `424242` ) ) ).

    DATA(lv_archive) = zabaputil_cl_util_ext=>zip_pack( lt_in ).
    DATA(lt_out) = zabaputil_cl_util_ext=>zip_unpack( lv_archive ).
...
CLASS ltcl_rtti_ext IMPLEMENTATION.
  ...
  METHOD dfies_by_table_basic.
    IF check_dfies_available( ) = abap_false.
      RETURN.
    ENDIF.
    DATA(lt_result) = zabaputil_cl_util_ext=>rtti_get_t_dfies_by_table_name( `Z2UI5_T_01` ).
  ...
  METHOD table_descr_basic.
    IF check_dfies_available( ) = abap_false.
      RETURN.
    ENDIF.
    DATA(lv_result) = zabaputil_cl_util_ext=>rtti_get_table_desrc( `Z2UI5_T_01` ).
...
CLASS ltcl_transport_ops IMPLEMENTATION.
  ...
  METHOD get_user_requests.
    DATA(lt_result) = zabaputil_cl_util_ext=>tr_get_user_requests( ) ##NEEDED.
  ENDMETHOD.
...
CLASS ltcl_xlsx_ops IMPLEMENTATION.
  ...
  METHOD xlsx_by_itab_basic.
    ...
    DATA(lv_result) = zabaputil_cl_util_ext=>conv_get_xlsx_by_itab( lt_tab ) ##NEEDED.
  ENDMETHOD.

  METHOD itab_by_xlsx_empty.
    DATA lv_xstring TYPE xstring.
    DATA lr_result TYPE REF TO data.
    zabaputil_cl_util_ext=>conv_get_itab_by_xlsx( EXPORTING val = lv_xstring
...
CLASS ltcl_source_ops IMPLEMENTATION.
  ...
  METHOD source_get_method_basic.
    DATA(lt_source) = zabaputil_cl_util_ext=>source_get_method(
