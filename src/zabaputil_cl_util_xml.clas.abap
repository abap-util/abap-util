CLASS zabaputil_cl_util_xml DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS __
      IMPORTING
        ...
        p             TYPE zabaputil_cl_context=>ty_t_name_value OPTIONAL
      RETURNING
        VALUE(result) TYPE REF TO zabaputil_cl_util_xml.
  ...
  PROTECTED SECTION.
    ...
    DATA mt_prop   TYPE SORTED TABLE OF zabaputil_cl_context=>ty_s_name_value WITH NON-UNIQUE KEY n.
  ...
  METHOD xml_get_parts.
    ...
    DATA(lv_tmp3) = REDUCE string( INIT val = `` FOR row IN mt_prop WHERE ( v <> `` ) "#EC CI_SORTSEQ
                          NEXT val = |{ val } { row-n }="{ escape( val    = COND string( WHEN row-v = abap_true
                                                                                         THEN `true`
                                                                                         ELSE row-v )
                                                                   format = zabaputil_cl_context=>cv_format_e_xml_attr ) }"| ).
