...
CLASS lcl_empty_filter IMPLEMENTATION.
  METHOD zabaputil_if_ajson_filter~keep_node.
    ...
    rv_keep = boolc(
      ( iv_visit = zabaputil_if_ajson_filter=>visit_type-value AND is_node-value IS NOT INITIAL ) OR
      ( iv_visit <> zabaputil_if_ajson_filter=>visit_type-value AND is_node-children > 0 ) ).
